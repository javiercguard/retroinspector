...
```{r data}
if (F) {
  saveRDS(snakemake, "~/path/snk3.rds")
  q()
  setwd("~/path")
...
observedMafTable = readRDS(snakemake@input[["observedMafTable"]])
...
hasAlleleleMask = observedMafTable$genotype %>% 
  lapply(`>=`, y = 1)

strictWithoutAllelesMask = mapply(function (x, y) {!x & y},
                                  hasAlleleleMask, observedMafTable$SUPP_mask_trusty,
                                  SIMPLIFY = F
)
...
strictWithFalsePositives = observedMafTable[strictWithFalsePositivesMask]
...
genotypeBenchmarkTable = data.table(
  ...
  falsePosPercent = nrow(strictWithFalsePositives) / nrow(observedMafTable[SUPP_trusty >= 1]) * 100,
  occurrences = sum(unlist(strictWithoutAlleles[strictWithFalsePositivesMask])),
  occurrencesPercent = sum(unlist(strictWithoutAlleles[strictWithFalsePositivesMask])) / sum(observedMafTable$SUPP_trusty) * 100,
  all0.0 = w,
  all0.0Occur = W)
```

- `r nrow(strictWithFalsePositives)` unique insertions that passed the stringent filtering.
(`r nrow(strictWithFalsePositives) / nrow(observedMafTable[SUPP_trusty >= 1]) * 100`% of the total unique insertions), had at least one occurrence genotyped as 0/0.
- These amount to `r sum(unlist(strictWithoutAlleles[strictWithFalsePositivesMask]))` total count (`r sum(unlist(strictWithoutAlleles[strictWithFalsePositivesMask])) / sum(observedMafTable$SUPP_trusty) * 100`% of filter-passing occurrences).
...
allelesWithoutStrictMask =
  mapply(function (x, y, z) {z >= 1 & x & !y},
         hasAlleleleMask, observedMafTable$SUPP_mask_trusty, observedMafTable$SUPP_trusty,
         SIMPLIFY = F
  )
...
strictWithFalseNegatives = observedMafTable[strictWithFalseNegativesMask]
...
genotypeBenchmarkTable[
  , `:=`(
    ...
    falseNegPercent = nrow(strictWithFalseNegatives) / nrow(observedMafTable[SUPP_trusty >= 1]) * 100,
    falseNegOcc = sum(unlist(allelesWithoutStrict[strictWithFalseNegativesMask])),
    callsThatPassed = sum(observedMafTable$SUPP_trusty),
    moreWrongThanRight = w
  )
]
```

- The strict criteria missed calls for `r nrow(strictWithFalseNegatives)` unique insertions (`r nrow(strictWithFalseNegatives) / nrow(observedMafTable[SUPP_trusty >= 1]) * 100`%).
- These mean a total of `r sum(unlist(allelesWithoutStrict[strictWithFalseNegativesMask]))` occurrences, out of `r sum(observedMafTable$SUPP_trusty)` calls that passed the stringent filter.
...
```{r table4, echo = F}
  ...
  min3WithoutAllelesMask = mapply(function (x, y) {!x & y},
           hasAlleleleMask, observedMafTable$SUPP_mask_min3,
           SIMPLIFY = F
           )
  ...
  laxWithFalsePositives = observedMafTable[laxWithFalsePositivesMask]
  ...
  genotypeBenchmarkLax = data.table(
    ...
    falsePosPercent = nrow(laxWithFalsePositives) / nrow(observedMafTable[SUPP_min3 >= 1]) * 100,
    occurrences = sum(unlist(min3WithoutAlleles[laxWithFalsePositivesMask])),
    occurrencesPercent = sum(unlist(min3WithoutAlleles[laxWithFalsePositivesMask])) / sum(observedMafTable$SUPP_min3) * 100,
    all0.0 = w,
    all0.0Occur = W
  )
  ...
  allelesWithoutMin3Mask =
    mapply(function (x, y, z) {z >= 1 & x & !y},
           hasAlleleleMask, observedMafTable$SUPP_mask_min3, observedMafTable$SUPP_min3,
           SIMPLIFY = F
           )
  ...
  laxWithFalseNegatives = observedMafTable[laxWithFalseNegativesMask]
  ...
  , `:=`(
    ...
    falseNegPercent = nrow(laxWithFalseNegatives) / nrow(observedMafTable[SUPP_min3 >= 1]) * 100,
    falseNegOcc = sum(unlist(allelesWithoutMin3[strictWithFalseNegativesMask])),
    callsThatPassed = sum(observedMafTable$SUPP_min3),
    moreWrongThanRight = w
  )]
...
observedMafTable = observedMafTable[!seqId %in% allFalsePositivesIds]
```

### Allele frequency

```{r results="asis"}
ubiquitous = observedMafTable[maf == 1]
...
- `r observedMafTable[maf >= 0.5 & SUPP_trusty > 0] %>% nrow()` TE insertions were found with an AF ≥ 0.5. 
...
copy = copy(observedMafTable[SUPP_trusty > 0])[, `:=`(seqnames = seqnames %>% as.numeric())]
qqman::manhattan(
  copy,
  chr = "seqnames",
  bp = "start", 
...
genotypedDeletions = readRDS(snakemake@input[["genotypedDeletions"]])
```
...
- The mean AF for deletions is `r genotypedDeletions[SUPP_trusty > 0]$maf %>% mean()` ± `r genotypedDeletions[SUPP_trusty > 0]$maf %>% sd()` (median of `r genotypedDeletions[SUPP_trusty > 0]$maf %>% median()`).
- With `r genotypedDeletions[SUPP_trusty > 0 & maf == 1] %>% nrow()` (`r genotypedDeletions[maf == 1 & SUPP_trusty > 0] %>% nrow()` of which has passed the stringent filtering) deletions appearing in a homozygous state in all patients (Fig. 7). On the other hand, `r genotypedDeletions[maf == 0 & SUPP_trusty > 0] %>% nrow()` deletions were genotyped as 0/0 on all cases, and have not been included in the rest of this section.

```{r}
genotypedDeletions = genotypedDeletions[!(maf == 0 & SUPP_trusty > 0)]
genotypedDeletions = genotypedDeletions[stri_order(seqnames, numeric = T)]
meDeletionsMin3 = meDeletionsMin3[id %in% genotypedDeletions$id]
```
...
copy = copy(genotypedDeletions[SUPP_trusty > 0])[, `:=`(
