This file documents the `rds` files generated by Retrosnake.
...
| support | For each insertion, the number of supporting reads in each sample | `list` of numeric vectors |
...
## observedMafTable.<allPrefix>.rds

This table contains the same information as `annotatedInsertionsMin3.<allPrefix>.rds`, with the addition of genotype information. 

| Column name | Meaning | Data type |
| --- | --- | --- |
| SUPP_geno | Number of patients that carry an INS, according to genotyping, i.e., those that have a 0/1 or 1/1 genotype | `num` |
| maf | Allele frequency of the INS (from 0 to 1) in the cohort on which the pipeline was run | `num` |
...
## genotypedDeletions.<allPrefix>.rds

Similarly to the genotyped insertions, this table contains genotype information for deletions. The two additional columns are explained below.
