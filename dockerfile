FROM condaforge/mambaforge:latest
LABEL io.github.snakemake.containerized="true"
LABEL io.github.snakemake.conda_env_hash="60f26aaf209a429a05cf7e9f06cc13dc3bd41198a39076a10957fe0c10735bd4"

# Step 1: Retrieve conda environments

# Conda environment:
#   source: env.yaml
#   prefix: /conda-envs/7481e9ef6a400c446aba4da0acca5028
#   channels:
#     - conda-forge
#     - bioconda
#   dependencies:
#     - _libgcc_mutex=0.1=conda_forge
#     - _openmp_mutex=4.5=2_gnu
#     - appdirs=1.4.4=pyh9f0ad1d_0
#     - bcftools=1.15.1=h0ea216a_0
#     - bedtools=2.30.0=h468198e_3
#     - biopython=1.80=py38h0a891b7_0
#     - brotli=1.0.9=h166bdaf_8
#     - brotli-bin=1.0.9=h166bdaf_8
#     - brotlipy=0.7.0=py38h0a891b7_1005
#     - bzip2=1.0.8=h7f98852_4
#     - c-ares=1.26.0=hd590300_0
#     - ca-certificates=2023.11.17=hbcca054_0
#     - cached-property=1.5.2=hd8ed1ab_1
#     - cached_property=1.5.2=pyha770c72_1
#     - certifi=2023.11.17=pyhd8ed1ab_0
#     - cffi=1.15.1=py38h4a40e3a_3
#     - charset-normalizer=2.1.1=pyhd8ed1ab_0
#     - cigar=0.1.3=pyh864c0ab_1
#     - click=8.1.3=py38h578d9bd_1
#     - colorama=0.4.6=pyhd8ed1ab_0
#     - coloredlogs=15.0.1=py38h578d9bd_2
#     - contourpy=1.0.7=py38hfbd4bf9_0
#     - cryptography=41.0.7=py38he241bfc_1
#     - curl=8.5.0=hca28451_0
#     - cutesv=2.0.2=pyhdfd78af_0
#     - cycler=0.11.0=pyhd8ed1ab_0
#     - cyvcf2=0.30.27=py38he403ad2_0
#     - entrez-direct=16.2=he881be0_1
#     - fonttools=4.38.0=py38h0a891b7_1
#     - freetype=2.12.1=hca18f0e_1
#     - fuzzywuzzy=0.18.0=pyhd8ed1ab_0
#     - gawk=5.1.0=h7f98852_0
#     - gettext=0.21.1=h27087fc_0
#     - gmp=6.1.2=hf484d3e_1000
#     - gnutls=3.5.19=h2a4e5f8_1
#     - gsl=2.7=he838d99_0
#     - h5py=3.8.0=nompi_py38h43830be_101
#     - hdf5=1.14.0=nompi_hb72d44e_103
#     - hmmer=3.3.2=h87f3376_2
#     - htslib=1.19.1=h81da01d_0
#     - humanfriendly=10.0=py38h578d9bd_4
#     - idna=3.4=pyhd8ed1ab_0
#     - importlib-metadata=6.0.0=pyha770c72_0
#     - joblib=1.2.0=pyhd8ed1ab_0
#     - jpeg=9e=h166bdaf_2
#     - k8=0.2.5=hd03093a_2
#     - keyutils=1.6.1=h166bdaf_0
#     - kiwisolver=1.4.4=py38h43d8883_1
#     - krb5=1.21.2=h659d440_0
#     - lcms2=2.12=hddcbb42_0
#     - ld_impl_linux-64=2.39=hcc3a1bd_1
#     - levenshtein=0.20.9=py38h8dc9893_0
#     - libaec=1.0.6=hcb278e6_1
#     - libblas=3.9.0=16_linux64_openblas
#     - libbrotlicommon=1.0.9=h166bdaf_8
#     - libbrotlidec=1.0.9=h166bdaf_8
#     - libbrotlienc=1.0.9=h166bdaf_8
#     - libcblas=3.9.0=16_linux64_openblas
#     - libcurl=8.5.0=hca28451_0
#     - libdeflate=1.18=h0b41bf4_0
#     - libedit=3.1.20191231=he28a2e2_2
#     - libev=4.33=h516909a_1
#     - libffi=3.4.2=h7f98852_5
#     - libgcc=7.2.0=h69d50b8_2
#     - libgcc-ng=12.2.0=h65d4601_19
#     - libgfortran-ng=12.2.0=h69a702a_19
#     - libgfortran5=12.2.0=h337968e_19
#     - libgomp=12.2.0=h65d4601_19
#     - libidn2=2.3.4=h166bdaf_0
#     - liblapack=3.9.0=16_linux64_openblas
#     - libnghttp2=1.58.0=h47da74e_1
#     - libnsl=2.0.0=h7f98852_0
#     - libopenblas=0.3.21=pthreads_h78a6416_3
#     - libpng=1.6.39=h753d276_0
#     - libsqlite=3.40.0=h753d276_0
#     - libssh2=1.11.0=h0841786_0
#     - libstdcxx-ng=12.2.0=h46fd767_19
#     - libtiff=4.2.0=hf544144_3
#     - libunistring=0.9.10=h7f98852_0
#     - libuuid=2.32.1=h7f98852_1000
#     - libwebp-base=1.2.4=h166bdaf_0
#     - libzlib=1.2.13=h166bdaf_4
#     - matplotlib-base=3.6.3=py38hd6c3c57_0
#     - matplotlib-venn=0.11.7=pyhd8ed1ab_0
#     - minimap=0.2=0
#     - minimap2=2.24=h7132678_1
#     - mosdepth=0.3.3=h37c5b7d_2
#     - munkres=1.1.4=pyh9f0ad1d_0
#     - ncurses=6.3=h27087fc_1
#     - nettle=3.3=0
#     - networkx=2.7=pyhd8ed1ab_0
#     - ngmlr=0.2.7=hd03093a_4
#     - numpy=1.24.2=py38h10c12cc_0
#     - olefile=0.46=pyh9f0ad1d_1
#     - openjpeg=2.4.0=hb52868f_1
#     - openssl=3.2.0=hd590300_1
#     - packaging=23.0=pyhd8ed1ab_0
#     - pandas=1.5.3=py38hdc8b05c_0
#     - pcre=8.45=h9c3ff4c_0
#     - perl=5.32.1=2_h7f98852_perl5
#     - perl-archive-tar=2.40=pl5321hdfd78af_0
#     - perl-carp=1.50=pl5321hd8ed1ab_0
#     - perl-common-sense=3.75=pl5321hd8ed1ab_0
#     - perl-compress-raw-bzip2=2.201=pl5321h166bdaf_0
#     - perl-compress-raw-zlib=2.202=pl5321h166bdaf_0
#     - perl-encode=3.19=pl5321h166bdaf_0
#     - perl-exporter=5.74=pl5321hd8ed1ab_0
#     - perl-exporter-tiny=1.002002=pl5321hd8ed1ab_0
#     - perl-extutils-makemaker=7.66=pl5321hd8ed1ab_0
#     - perl-io-compress=2.201=pl5321h87f3376_0
#     - perl-io-zlib=1.14=pl5321hdfd78af_0
#     - perl-json=4.10=pl5321hdfd78af_0
#     - perl-json-xs=2.34=pl5321h9f5acd7_5
#     - perl-list-moreutils=0.430=pl5321hdfd78af_0
#     - perl-list-moreutils-xs=0.430=pl5321hec16e2b_1
#     - perl-parent=0.241=pl5321hd8ed1ab_0
#     - perl-pathtools=3.75=pl5321h166bdaf_0
#     - perl-scalar-list-utils=1.63=pl5321h166bdaf_0
#     - perl-storable=3.15=pl5321h166bdaf_0
#     - perl-types-serialiser=1.01=pl5321hdfd78af_0
#     - pillow=8.2.0=py38ha0e1e83_1
#     - pip=21.3.1=pyhd8ed1ab_0
#     - pooch=1.6.0=pyhd8ed1ab_0
#     - psutil=5.9.4=py38h0a891b7_0
#     - pycparser=2.21=pyhd8ed1ab_0
#     - pyopenssl=23.3.0=pyhd8ed1ab_0
#     - pyparsing=3.0.9=pyhd8ed1ab_0
#     - pysam=0.22.0=py38h15b938a_0
#     - pysocks=1.7.1=py38h578d9bd_5
#     - pyspoa=0.2.1=py38he0f268d_0
#     - python=3.8.16=he550d4f_1_cpython
#     - python-dateutil=2.8.2=pyhd8ed1ab_0
#     - python-levenshtein=0.20.9=pyhd8ed1ab_0
#     - python_abi=3.8=3_cp38
#     - pytz=2022.7.1=pyhd8ed1ab_0
#     - pyvcf=0.6.8=py38h32f6830_1002
#     - rapidfuzz=2.13.7=py38h8dc9893_0
#     - readline=8.1.2=h0f457ee_0
#     - repeatmasker=4.1.2.p1=pl5321hdfd78af_1
#     - requests=2.28.2=pyhd8ed1ab_0
#     - rmblast=2.11.0=h6200dbe_0
#     - samtools=1.15.1=h1170115_0
#     - scipy=1.10.0=py38h10c12cc_2
#     - setuptools=67.1.0=pyhd8ed1ab_0
#     - six=1.16.0=pyh6c4a22f_0
#     - sniffles=1.0.12=h8b12597_1
#     - sqlite=3.40.0=h4ff8645_0
#     - surpyvor=0.12.0=pyhdfd78af_0
#     - survivor=1.0.7=hd03093a_2
#     - svim=1.4.2=py_0
#     - tclap=1.2.3=h9c3ff4c_0
#     - tk=8.6.12=h27826a3_0
#     - tornado=6.2=py38h0a891b7_1
#     - tqdm=4.66.1=pyhd8ed1ab_0
#     - trf=4.09.1=hec16e2b_2
#     - typing_extensions=4.1.1=pyha770c72_0
#     - unicodedata2=15.0.0=py38h0a891b7_0
#     - upsetplot=0.8.0=pyhd8ed1ab_0
#     - urllib3=1.26.14=pyhd8ed1ab_0
#     - wget=1.20.3=ha35d2d1_1
#     - wheel=0.37.1=pyhd8ed1ab_0
#     - wtdbg=2.5=h5b5514e_2
#     - xz=5.2.6=h166bdaf_0
#     - zipp=3.6.0=pyhd8ed1ab_0
#     - zlib=1.2.13=h166bdaf_4
#     - zstd=1.5.5=hfc55251_0
RUN mkdir -p /conda-envs/7481e9ef6a400c446aba4da0acca5028
COPY env.yaml /conda-envs/7481e9ef6a400c446aba4da0acca5028/environment.yaml

# Conda environment:
#   source: r.yaml
#   prefix: /conda-envs/25917e26b834594ac7346922950e2a34
#   channels:
#     - conda-forge
#     - bioconda
#     - defaults
#   dependencies:
#     - _libgcc_mutex=0.1=conda_forge
#     - _openmp_mutex=4.5=2_gnu
#     - _r-mutex=1.0.1=anacondar_1
#     - argcomplete=2.0.0=pyhd8ed1ab_0
#     - binutils_impl_linux-64=2.40=hf600244_0
#     - bioconductor-annotationdbi=1.60.0=r42hdfd78af_0
#     - bioconductor-annotationhub=3.6.0=r42hdfd78af_0
#     - bioconductor-annotatr=1.24.0=r42hdfd78af_0
#     - bioconductor-biobase=2.58.0=r42hc0cfd56_0
#     - bioconductor-biocfilecache=2.6.0=r42hdfd78af_0
#     - bioconductor-biocgenerics=0.44.0=r42hdfd78af_0
#     - bioconductor-biocio=1.8.0=r42hdfd78af_0
#     - bioconductor-biocparallel=1.32.5=r42hc247a5b_0
#     - bioconductor-biocversion=3.16.0=r42hdfd78af_0
#     - bioconductor-biomart=2.54.0=r42hdfd78af_0
#     - bioconductor-biostrings=2.66.0=r42hc0cfd56_0
#     - bioconductor-bsgenome=1.66.1=r42hdfd78af_0
#     - bioconductor-clusterprofiler=4.6.0=r42hdfd78af_0
#     - bioconductor-data-packages=20230202=hdfd78af_0
#     - bioconductor-delayedarray=0.24.0=r42hc0cfd56_0
#     - bioconductor-dose=3.24.0=r42hdfd78af_0
#     - bioconductor-enrichplot=1.18.0=r42hdfd78af_0
#     - bioconductor-fgsea=1.24.0=r42hf17093f_1
#     - bioconductor-genomeinfodb=1.34.8=r42hdfd78af_0
#     - bioconductor-genomeinfodbdata=1.2.9=r42hdfd78af_0
#     - bioconductor-genomicalignments=1.34.0=r42hc0cfd56_0
#     - bioconductor-genomicfeatures=1.50.2=r42hdfd78af_0
#     - bioconductor-genomicranges=1.50.0=r42hc0cfd56_0
#     - bioconductor-ggtree=3.6.0=r42hdfd78af_0
#     - bioconductor-go.db=3.16.0=r42hdfd78af_0
#     - bioconductor-gosemsim=2.24.0=r42hf17093f_1
#     - bioconductor-graph=1.76.0=r42ha9d7317_1
#     - bioconductor-hdo.db=0.99.1=r42hdfd78af_0
#     - bioconductor-interactivedisplaybase=1.36.0=r42hdfd78af_0
#     - bioconductor-iranges=2.32.0=r42hc0cfd56_0
#     - bioconductor-keggrest=1.38.0=r42hdfd78af_0
#     - bioconductor-matrixgenerics=1.10.0=r42hdfd78af_0
#     - bioconductor-org.hs.eg.db=3.16.0=r42hdfd78af_0
#     - bioconductor-qvalue=2.30.0=r42hdfd78af_0
#     - bioconductor-regioner=1.30.0=r42hdfd78af_0
#     - bioconductor-rgraphviz=2.42.0=r42hf17093f_1
#     - bioconductor-rhtslib=2.0.0=r42hc0cfd56_0
#     - bioconductor-rsamtools=2.14.0=r42hc247a5b_0
#     - bioconductor-rtracklayer=1.58.0=r42h58c1800_2
#     - bioconductor-s4vectors=0.36.0=r42hc0cfd56_0
#     - bioconductor-summarizedexperiment=1.28.0=r42hdfd78af_0
#     - bioconductor-topgo=2.50.0=r42hdfd78af_0
#     - bioconductor-treeio=1.22.0=r42hdfd78af_0
#     - bioconductor-txdb.hsapiens.ucsc.hg38.knowngene=3.16.0=r42hdfd78af_0
#     - bioconductor-xvector=0.38.0=r42hc0cfd56_0
#     - bioconductor-zlibbioc=1.44.0=r42hc0cfd56_0
#     - bwidget=1.9.14=ha770c72_1
#     - bzip2=1.0.8=h7f98852_4
#     - c-ares=1.18.1=h7f98852_0
#     - ca-certificates=2023.11.17=hbcca054_0
#     - cairo=1.16.0=ha61ee94_1014
#     - curl=8.1.2=h409715c_0
#     - expat=2.5.0=h27087fc_0
#     - font-ttf-dejavu-sans-mono=2.37=hab24e00_0
#     - font-ttf-inconsolata=3.000=h77eed37_0
#     - font-ttf-source-code-pro=2.038=h77eed37_0
#     - font-ttf-ubuntu=0.83=hab24e00_0
#     - fontconfig=2.14.2=h14ed4e7_0
#     - fonts-conda-ecosystem=1=0
#     - fonts-conda-forge=1=0
#     - freetype=2.12.1=hca18f0e_1
#     - fribidi=1.0.10=h36c2ea0_0
#     - gcc_impl_linux-64=12.2.0=hcc96c02_19
#     - gettext=0.21.1=h27087fc_0
#     - gfortran_impl_linux-64=12.2.0=h55be85b_19
#     - glpk=5.0=h445213a_0
#     - gmp=6.3.0=h59595ed_0
#     - graphite2=1.3.13=h58526e2_1001
#     - gsl=2.7=he838d99_0
#     - gxx_impl_linux-64=12.2.0=hcc96c02_19
#     - harfbuzz=6.0.0=h8e241bc_0
#     - icu=70.1=h27087fc_0
#     - importlib-metadata=4.13.0=pyha770c72_0
#     - importlib_metadata=4.13.0=hd8ed1ab_0
#     - jpeg=9e=h0b41bf4_3
#     - jq=1.6=h36c2ea0_1000
#     - kernel-headers_linux-64=2.6.32=he073ed8_15
#     - keyutils=1.6.1=h166bdaf_0
#     - krb5=1.20.1=h81ceb04_0
#     - ld_impl_linux-64=2.40=h41732ed_0
#     - lerc=4.0.0=h27087fc_0
#     - libblas=3.9.0=16_linux64_openblas
#     - libcblas=3.9.0=16_linux64_openblas
#     - libcurl=8.1.2=h409715c_0
#     - libdeflate=1.17=h0b41bf4_0
#     - libedit=3.1.20191231=he28a2e2_2
#     - libev=4.33=h516909a_1
#     - libffi=3.4.2=h7f98852_5
#     - libgcc-devel_linux-64=12.2.0=h3b97bd3_19
#     - libgcc-ng=12.2.0=h65d4601_19
#     - libgfortran-ng=12.2.0=h69a702a_19
#     - libgfortran5=12.2.0=h337968e_19
#     - libglib=2.74.1=h606061b_1
#     - libgomp=12.2.0=h65d4601_19
#     - libiconv=1.17=h166bdaf_0
#     - liblapack=3.9.0=16_linux64_openblas
#     - libnghttp2=1.52.0=h61bc06f_0
#     - libnsl=2.0.0=h7f98852_0
#     - libopenblas=0.3.21=pthreads_h78a6416_3
#     - libpng=1.6.39=h753d276_0
#     - libsanitizer=12.2.0=h46fd767_19
#     - libsqlite=3.40.0=h753d276_0
#     - libssh2=1.11.0=h0841786_0
#     - libstdcxx-devel_linux-64=12.2.0=h3b97bd3_19
#     - libstdcxx-ng=12.2.0=h46fd767_19
#     - libtiff=4.5.0=h6adf6a1_2
#     - libuuid=2.32.1=h7f98852_1000
#     - libwebp-base=1.2.4=h166bdaf_0
#     - libxcb=1.13=h7f98852_1004
#     - libxml2=2.10.3=h7463322_0
#     - libzlib=1.2.13=h166bdaf_4
#     - make=4.3=hd18ef5c_1
#     - ncurses=6.3=h27087fc_1
#     - oniguruma=6.9.8=h166bdaf_0
#     - openssl=3.1.4=hd590300_0
#     - pandoc=2.19.2=h32600fe_1
#     - pango=1.50.13=hd33c08f_0
#     - pcre2=10.40=hc3806b6_0
#     - pip=23.0.1=pyhd8ed1ab_0
#     - pixman=0.40.0=h36c2ea0_0
#     - pthread-stubs=0.4=h36c2ea0_1001
#     - python=3.11.0=he550d4f_1_cpython
#     - python_abi=3.11=3_cp311
#     - pyyaml=6.0=py311hd4cff14_5
#     - r-ape=5.7_1=r42h08d816e_1
#     - r-aplot=0.2.2=r42hc72bb7e_0
#     - r-askpass=1.1=r42h06615bd_3
#     - r-assertthat=0.2.1=r42hc72bb7e_3
#     - r-base=4.2.2=ha7d60f8_3
#     - r-base64enc=0.1_3=r42h06615bd_1005
#     - r-bh=1.81.0_1=r42hc72bb7e_0
#     - r-biocmanager=1.30.19=r42hc72bb7e_0
#     - r-bit=4.0.5=r42h06615bd_0
#     - r-bit64=4.0.5=r42h06615bd_1
#     - r-bitops=1.0_7=r42h06615bd_1
#     - r-blob=1.2.3=r42hc72bb7e_1
#     - r-bslib=0.4.2=r42hc72bb7e_0
#     - r-cachem=1.0.6=r42h06615bd_1
#     - r-calibrate=1.7.7=r42hc72bb7e_1
#     - r-cli=3.6.0=r42h38f115c_0
#     - r-clipr=0.8.0=r42hc72bb7e_1
#     - r-codetools=0.2_19=r42hc72bb7e_0
#     - r-colorspace=2.1_0=r42h133d619_0
#     - r-commonmark=1.8.1=r42h06615bd_0
#     - r-cowplot=1.1.1=r42hc72bb7e_2
#     - r-cpp11=0.4.3=r42hc72bb7e_0
#     - r-crayon=1.5.2=r42hc72bb7e_1
#     - r-crosstalk=1.2.0=r42hc72bb7e_1
#     - r-crul=1.3=r42h785f33e_1
#     - r-curl=4.3.3=r42h06615bd_1
#     - r-data.table=1.14.8=r42h133d619_0
#     - r-dbi=1.1.3=r42hc72bb7e_1
#     - r-dbplyr=2.3.0=r42hc72bb7e_0
#     - r-digest=0.6.31=r42h38f115c_0
#     - r-downloader=0.4=r42hc72bb7e_1005
#     - r-dplyr=1.1.0=r42h38f115c_0
#     - r-dt=0.27=r42hc72bb7e_0
#     - r-ellipsis=0.3.2=r42h06615bd_1
#     - r-evaluate=0.20=r42hc72bb7e_0
#     - r-fansi=1.0.4=r42h133d619_0
#     - r-farver=2.1.1=r42h7525677_1
#     - r-fastmap=1.1.0=r42h7525677_1
#     - r-fastmatch=1.1_4=r42h57805ef_0
#     - r-filelock=1.0.2=r42h06615bd_1003
#     - r-flextable=0.8.6=r42hc72bb7e_0
#     - r-fontawesome=0.5.0=r42hc72bb7e_0
#     - r-fontbitstreamvera=0.1.1=r42hc72bb7e_1004
#     - r-fontliberation=0.1.0=r42hc72bb7e_1004
#     - r-fontquiver=0.2.1=r42hc72bb7e_1004
#     - r-formatr=1.14=r42hc72bb7e_0
#     - r-fs=1.6.1=r42h38f115c_0
#     - r-futile.logger=1.4.3=r42hc72bb7e_1004
#     - r-futile.options=1.0.1=r42hc72bb7e_1003
#     - r-gdtools=0.3.1=r42he0ce631_0
#     - r-generics=0.1.3=r42hc72bb7e_1
#     - r-gfonts=0.2.0=r42hc72bb7e_0
#     - r-ggforce=0.4.1=r42ha503ecb_2
#     - r-ggfun=0.1.3=r42hc72bb7e_0
#     - r-ggnewscale=0.4.9=r42hc72bb7e_1
#     - r-ggplot2=3.4.1=r42hc72bb7e_0
#     - r-ggplotify=0.1.2=r42hc72bb7e_0
#     - r-ggraph=2.1.0=r42ha503ecb_2
#     - r-ggrepel=0.9.4=r42ha503ecb_0
#     - r-glue=1.6.2=r42h06615bd_1
#     - r-graphlayouts=1.0.2=r42ha503ecb_0
#     - r-gridextra=2.3=r42hc72bb7e_1005
#     - r-gridgraphics=0.5_1=r42hc72bb7e_2
#     - r-gson=0.1.0=r42hc72bb7e_1
#     - r-gtable=0.3.1=r42hc72bb7e_1
#     - r-hexbin=1.28.3=r42hac0b197_0
#     - r-highr=0.10=r42hc72bb7e_0
#     - r-hms=1.1.2=r42hc72bb7e_1
#     - r-htmltools=0.5.4=r42h38f115c_0
#     - r-htmlwidgets=1.6.1=r42hc72bb7e_0
#     - r-httpcode=0.3.0=r42ha770c72_2
#     - r-httpuv=1.6.9=r42h38f115c_0
#     - r-httr=1.4.4=r42hc72bb7e_1
#     - r-igraph=1.4.2=r42h65ed38e_0
#     - r-isoband=0.2.7=r42h38f115c_1
#     - r-jquerylib=0.1.4=r42hc72bb7e_1
#     - r-jsonlite=1.8.4=r42h133d619_0
#     - r-knitr=1.42=r42hc72bb7e_1
#     - r-labeling=0.4.2=r42hc72bb7e_2
#     - r-lambda.r=1.2.4=r42hc72bb7e_2
#     - r-later=1.3.0=r42h7525677_1
#     - r-lattice=0.20_45=r42h06615bd_1
#     - r-lazyeval=0.2.2=r42h06615bd_3
#     - r-lifecycle=1.0.3=r42hc72bb7e_1
#     - r-magrittr=2.0.3=r42h06615bd_1
#     - r-markdown=1.5=r42hc72bb7e_0
#     - r-mass=7.3_58.2=r42h133d619_0
#     - r-matrix=1.5_3=r42h5f7b363_0
#     - r-matrixstats=0.63.0=r42h06615bd_0
#     - r-memoise=2.0.1=r42hc72bb7e_1
#     - r-mgcv=1.8_41=r42h5f7b363_0
#     - r-mime=0.12=r42h06615bd_1
#     - r-munsell=0.5.0=r42hc72bb7e_1005
#     - r-nlme=3.1_162=r42hac0b197_0
#     - r-officer=0.6.0=r42hc72bb7e_0
#     - r-openssl=2.1.1=r42hb353fa6_0
#     - r-patchwork=1.1.2=r42hc72bb7e_1
#     - r-pillar=1.8.1=r42hc72bb7e_1
#     - r-pkgconfig=2.0.3=r42hc72bb7e_2
#     - r-plogr=0.2.0=r42hc72bb7e_1004
#     - r-plotly=4.10.3=r42hc72bb7e_0
#     - r-plyr=1.8.8=r42h7525677_0
#     - r-png=0.1_8=r42h10cf519_0
#     - r-polyclip=1.10_6=r42ha503ecb_0
#     - r-prettyunits=1.1.1=r42hc72bb7e_2
#     - r-progress=1.2.2=r42hc72bb7e_3
#     - r-promises=1.2.0.1=r42h7525677_1
#     - r-purrr=1.0.1=r42h133d619_0
#     - r-qqman=0.1.8=r42hc72bb7e_1
#     - r-r.methodss3=1.8.2=r42hc72bb7e_1
#     - r-r.oo=1.25.0=r42hc72bb7e_1
#     - r-r.utils=2.12.2=r42hc72bb7e_0
#     - r-r6=2.5.1=r42hc72bb7e_1
#     - r-ragg=1.2.5=r42hd65d3ba_0
#     - r-rappdirs=0.3.3=r42h06615bd_1
#     - r-rcolorbrewer=1.1_3=r42h785f33e_1
#     - r-rcpp=1.0.10=r42h38f115c_0
#     - r-rcpparmadillo=0.12.6.4.0=r42h08d816e_0
#     - r-rcppeigen=0.3.3.9.4=r42h08d816e_0
#     - r-rcurl=1.98_1.12=r42hf9611b0_2
#     - r-readr=2.1.4=r42h38f115c_0
#     - r-reshape2=1.4.4=r42h7525677_2
#     - r-restfulr=0.0.15=r42h73dbb54_1
#     - r-rjson=0.2.21=r42h7525677_2
#     - r-rlang=1.0.6=r42h7525677_1
#     - r-rmarkdown=2.20=r42hc72bb7e_0
#     - r-rsqlite=2.2.20=r42h38f115c_0
#     - r-sass=0.4.5=r42h38f115c_0
#     - r-scales=1.2.1=r42hc72bb7e_1
#     - r-scatterpie=0.2.1=r42hc72bb7e_1
#     - r-servr=0.25=r42hc72bb7e_0
#     - r-shadowtext=0.1.2=r42hc72bb7e_2
#     - r-shiny=1.7.4=r42h785f33e_0
#     - r-snow=0.4_4=r42hc72bb7e_1
#     - r-sourcetools=0.1.7_1=r42h38f115c_0
#     - r-sparsem=1.81=r42h8da6f51_1
#     - r-stringi=1.7.12=r42h1ae9187_0
#     - r-stringr=1.5.0=r42h785f33e_0
#     - r-sys=3.4.1=r42h06615bd_0
#     - r-systemfonts=1.0.4=r42h0ff29ef_1
#     - r-textshaping=0.3.6=r42hbb20487_4
#     - r-tibble=3.1.8=r42h06615bd_1
#     - r-tidygraph=1.2.3=r42ha503ecb_1
#     - r-tidyr=1.3.0=r42ha503ecb_1
#     - r-tidyselect=1.2.0=r42hc72bb7e_0
#     - r-tidytree=0.4.5=r42hc72bb7e_0
#     - r-tinytex=0.44=r42hc72bb7e_0
#     - r-triebeard=0.4.1=r42h38f115c_0
#     - r-tweenr=2.0.2=r42ha503ecb_2
#     - r-tzdb=0.3.0=r42h7525677_1
#     - r-urltools=1.7.3=r42h7525677_3
#     - r-utf8=1.2.3=r42h133d619_0
#     - r-uuid=1.1_0=r42h06615bd_1
#     - r-vctrs=0.5.2=r42h38f115c_0
#     - r-viridis=0.6.4=r42hc72bb7e_0
#     - r-viridislite=0.4.1=r42hc72bb7e_1
#     - r-vroom=1.6.1=r42h38f115c_0
#     - r-withr=2.5.0=r42hc72bb7e_1
#     - r-xaringan=0.28=r42hc72bb7e_0
#     - r-xfun=0.37=r42h38f115c_0
#     - r-xml=3.99_0.13=r42hed7f998_0
#     - r-xml2=1.3.3=r42h044e5c7_2
#     - r-xtable=1.8_4=r42hc72bb7e_4
#     - r-yaml=2.3.7=r42h133d619_0
#     - r-yulab.utils=0.1.0=r42hc72bb7e_0
#     - r-zip=2.2.2=r42h06615bd_0
#     - readline=8.1.2=h0f457ee_0
#     - sed=4.8=he412f7d_0
#     - setuptools=67.4.0=pyhd8ed1ab_0
#     - sysroot_linux-64=2.12=he073ed8_15
#     - tk=8.6.12=h27826a3_0
#     - tktable=2.10=hb7b940f_3
#     - toml=0.10.2=pyhd8ed1ab_0
#     - tzdata=2022g=h191b570_0
#     - wheel=0.38.4=pyhd8ed1ab_0
#     - xmltodict=0.13.0=pyhd8ed1ab_0
#     - xorg-kbproto=1.0.7=h7f98852_1002
#     - xorg-libice=1.0.10=h7f98852_0
#     - xorg-libsm=1.2.3=hd9c2040_1000
#     - xorg-libx11=1.7.2=h7f98852_0
#     - xorg-libxau=1.0.9=h7f98852_0
#     - xorg-libxdmcp=1.1.3=h7f98852_0
#     - xorg-libxext=1.3.4=h0b41bf4_2
#     - xorg-libxrender=0.9.10=h7f98852_1003
#     - xorg-libxt=1.2.1=h7f98852_2
#     - xorg-renderproto=0.11.1=h7f98852_1002
#     - xorg-xextproto=7.3.0=h0b41bf4_1003
#     - xorg-xproto=7.0.31=h7f98852_1007
#     - xz=5.2.6=h166bdaf_0
#     - yaml=0.2.5=h7f98852_2
#     - yq=3.1.1=pyhd8ed1ab_0
#     - zipp=3.14.0=pyhd8ed1ab_0
#     - zlib=1.2.13=h166bdaf_4
#     - zstd=1.5.2=h3eb15da_6
RUN mkdir -p /conda-envs/25917e26b834594ac7346922950e2a34
COPY r.yaml /conda-envs/25917e26b834594ac7346922950e2a34/environment.yaml

# Step 2: Generate conda environments

RUN mamba env create --prefix /conda-envs/7481e9ef6a400c446aba4da0acca5028 --file /conda-envs/7481e9ef6a400c446aba4da0acca5028/environment.yaml && \
    mamba env create --prefix /conda-envs/25917e26b834594ac7346922950e2a34 --file /conda-envs/25917e26b834594ac7346922950e2a34/environment.yaml && \
    mamba clean --all -y
